...
### With the Fig. 6 array and real speech

`src/patent_circular_speech.py` — the patent's preferred array (three mics
on an 8 cm-radius circle plus one at the centre), two talkers in a
reverberant room (RT60 0.3 s, pyroomacoustics), speech synthesized with
macOS `say` into `data/`. The full Fig. 7 process: STFT → per-bin `R` →
MVDR scan over 0–360° averaged over 300–4000 Hz → steering direction =
scan peak **+3° deliberate error** → per-bin weights → ISTFT. Reference for
SNR is the target-only image at the centre mic (reverb included).

![Patent with speech](plots/patent_circular_speech.png)

```
target 30° (0 dB), interferer 150° (−6 dB), sensor noise −30 dB, steered to 33°, beam width 8°
  centre mic (input)   7.3 dB
  delay-and-sum        8.3 dB
  MVDR δ=1e-4          2.0 dB     ← self-null: 5 dB *worse* than a single mic
  MVDR δ=0.1           9.5 dB
  patent               9.5 dB
```

- The DOA scan finds 30° exactly, but with a 16 cm aperture and 4 mics the
  peak is very broad (polar panel) — the array cannot resolve two talkers
  separated by 120° sharply, which is why a small device wants robustness
  more than resolution.
- Unloaded MVDR self-nulls the target even at 0° error and in an anechoic
  room (checked): the target is 30 dB above the noise, so the residual
  mismatch from reverberation, 1/r amplitude differences across the ring
  and fractional-delay interpolation is enough to trigger it. This is the
  patent's ¶14 scenario with real speech.
- The patent and loaded MVDR tie here; with 4 mics the three constraints
  leave one degree of freedom, so the extra beam-width guarantee costs
  nothing over plain loading but also cannot buy more. Its advantage over
  loading shows up with more mics and larger DOA errors (§11 above).
- Listen: `results/patent_circular_*.wav` (input, clean target, and each
  output; not committed — regenerated by the script).
